...
public class DataUtils {
				...
				if(leftDatasetName.equals(datasetName)){
					if(leftCell.getBindData()!=null){
						...
						if(cell.getBindData() == null){
							cell.setBindData(leftCell.getBindData());
						}
					}
				}
			}
	...
	public static DatasetExpression fetchDatasetExpression(Value value){
		if(value instanceof ExpressionValue){
			...
			if(expr instanceof DatasetExpression){
				...
				DatasetExpression targetExpr=buildDatasetExpression(blockExpr);
				return targetExpr;
			}
			else{
				return null;
	...
	private static DatasetExpression buildDatasetExpression(JoinExpression joinExpr){
		...
		for(BaseExpression baseExpr:expressions){
			...
			}else if(baseExpr instanceof FunctionExpression){
				FunctionExpression childExpr=(FunctionExpression)baseExpr;
				return buildDatasetExpression(childExpr);
			}
		}
		return null;
...
//	private static DatasetExpression buildDatasetExpression(IfExpression joinExpr){
//		List<BaseExpression> expressions=joinExpr.getExpressions();
//		for(BaseExpression baseExpr:expressions){
//			if(baseExpr instanceof DatasetExpression){
//				return (DatasetExpression)baseExpr;
...
//	}
	...
	private static DatasetExpression buildDatasetExpression(FunctionExpression joinExpr){
		if(ToolUtils.isNotEmpty(joinExpr.getExpressions())){
			List<BaseExpression> expressions=joinExpr.getExpressions();
			for(BaseExpression baseExpr:expressions){
				if(baseExpr instanceof DatasetExpression){
					return (DatasetExpression)baseExpr;
	...
	private static DatasetExpression buildDatasetExpression(ExpressionBlock blockExpr){
		List<Expression> expressions=blockExpr.getExpressionList();
		for(Expression baseExpr:expressions){
			if(baseExpr instanceof DatasetExpression){
				return (DatasetExpression)baseExpr;
			}else if(baseExpr instanceof ExpressionBlock){
				ExpressionBlock childExpr=(ExpressionBlock)baseExpr;
				return buildDatasetExpression(childExpr);
			}else if(baseExpr instanceof JoinExpression){
				JoinExpression childExpr=(JoinExpression)baseExpr;
				return buildDatasetExpression(childExpr);
			}
			else if(baseExpr instanceof IfExpression){
				IfExpression childExpr=(IfExpression)baseExpr;
				return buildDatasetIFExpression(childExpr);
			}
		}
		return null;
	}

	private static DatasetExpression buildDatasetIFExpression(IfExpression blockExpr){
		ExpressionConditionList expressionConditionList  = blockExpr.getConditionList();
		List<ExpressionCondition> conditions = expressionConditionList.getConditions();
		for(ExpressionCondition expressionCondition:conditions){
			Expression left = expressionCondition.getLeft();
			if(left instanceof ParenExpression){
				ParenExpression parenExpr=(ParenExpression)left;
				DatasetExpression targetExpr=buildDatasetExpression(parenExpr);
				if(targetExpr != null){
					return targetExpr;
				}
			}
			Expression right = expressionCondition.getRight();
			if(right instanceof ParenExpression){
				ParenExpression parenExpr=(ParenExpression)right;
				DatasetExpression targetExpr=buildDatasetExpression(parenExpr);
				return targetExpr;
			}
		}
