...
public class PdfProducer implements Producer {
	...
	public void produce(Report report,OutputStream outputStream) {
		...
		// 创建 Document 、 指定边距、创建空白pdf
		Document document=new Document(pageSize,leftMargin,rightMargin,topMargin,bottomMargin);
		try{
			// 创建一个 PdfWriter 对象
			PdfWriter writer=PdfWriter.getInstance(document,output);
			document.addAuthor(StrUtil.join(StrUtil.EMPTY, ToolUtils.__GENERATED__));
			document.addTitle(report.getConfig().getDownName());
			...
			document.close();
			// 检查是否需要添加水印
			if(!ToolUtils.isEmpty(paper.getWatermark())){
				ByteArrayOutputStream tempPutput = new ByteArrayOutputStream();
				try{
					InputStream inputStream = IoUtil.toStream(output.toByteArray());
					addStringWaterMark(inputStream,
							outputStream,paper.getWatermark(),
							2,2,0.4f,45f,25,
							BaseColor.BLACK);
					return;
				}catch (Exception e){
	...
	/**
	 * 添加斜角排列、全屏多个重复的花式文字水印
	 *
	 * @param inputStream       需要加水印的PDF读取输入流
	 * @param output            输出生成PDF的输出流
	 * @param waterMarkString   水印字符
	 * @param xAmount           x轴重复数量
	 * @param yAmount           y轴重复数量
	 * @param opacity           水印透明度
	 ...
	 */
	public void addStringWaterMark(InputStream inputStream,
										  OutputStream output,
										  ...
										  BaseColor color) throws IOException, DocumentException {
		try {

			PdfReader reader = new PdfReader(inputStream);
			PdfStamper stamper = new PdfStamper(reader, output);
			Font baseFont = FontBuilder.getFont("宋体", waterMarkFontSize, false, false,false);
			...
			for (int i = 1; i < total; i++) {
				...
				// 计算水印每个单位步长X,Y
				float x = pageRect.getWidth() / xAmount;
				float y = pageRect.getHeight() / yAmount;
				...
				over.setFontAndSize(baseFont.getBaseFont(), waterMarkFontSize);

				for (int n = 0; n < xAmount + 1; n++) {
					for (int m = 0; m < yAmount + 1; m++) {
