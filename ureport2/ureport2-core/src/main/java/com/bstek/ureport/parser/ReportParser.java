...
public class ReportParser {
	...
	public ReportDefinition parse(InputStream inputStream,String file) {
		...
		try{
			...
			// 图片信息
			List<ImgDefinition> imgs=new ArrayList<ImgDefinition>();
			// 列样本
			...
			for(Object obj:element.elements()){
				...
				if(parser!=null){
					...
					}else if(target instanceof ImgDefinition){
						imgs.add((ImgDefinition)target);
		...
		for(CellDefinition cell:cells){
			...
			if(StringUtils.isNotBlank(downParentCellName)){
				...
				if(rowN < report.getRows().size()){
					...
					if(ToolUtils.isNotEmpty(targetCell)){
