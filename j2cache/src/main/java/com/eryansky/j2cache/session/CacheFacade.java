...
public class CacheFacade extends RedisPubSubAdapter<String, String> implements Closeable, CacheExpiredListener {
    ...
    public CacheFacade(int maxSizeInMemory, int maxAge, Properties redisConf, boolean discardNonSerializable)  {
        ...
        poolConfig.setTestOnBorrow(Boolean.parseBoolean(redisConf.getProperty("testOnBorrow", "true"))); // 在每次借用连接时进行有效性检查
        poolConfig.setTestOnReturn(Boolean.parseBoolean(redisConf.getProperty("testOnReturn", "false"))); // 在每次归还连接时进行有效性检查
        ...
        return keys().stream()
                ...
                .filter(sessionObject -> {
                    try {
                        ...
                        if (sessionData instanceof Identifiable identifiable) {
                            return session_id.equals(identifiable.getId());
                        }
                        return false;
                    } catch (Exception e) {
