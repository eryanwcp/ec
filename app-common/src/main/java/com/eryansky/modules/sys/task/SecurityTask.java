...
public class SecurityTask {
    ...
    public void checkRiskUserDevice(String loginName, String deviceCode, String ip, String userAgent) {
        ...
        if (entity == null) {
            ...
            msg.append("安全预警：异常设备登录尝试！时间：" + DateUtils.getCurrentDateTime() + "，用户：" + loginName + "，IP：" + ip + ",设备：" + userAgent);
    ...
    public void checkRiskUserDevice(SessionInfo sessionInfo) {
        ...
        if (entity == null) {
            StringBuffer msg = new StringBuffer();
            msg.append("安全预警：异常设备登录成功！时间：" + DateUtils.getCurrentDateTime() + "，用户：" + sessionInfo.getLoginName() + "，IP：" + sessionInfo.getIp() + ",设备：" + sessionInfo.getUserAgent());
            sendMessage(msg.toString());
        }
    }
