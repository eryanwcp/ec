...
//        CacheUtils.cacheReportDefinition(file, reportDefinition);
    ...
    public HtmlReport exportHtml(String file,String contextPath,Map<String, Object> parameters, int pageIndex) {
        ...
        int totalPages = 0;
        if(ObjectUtils.isNotEmpty(parameters.get("__paging")) &&
    ...
    public void setSqlFilter(ReportDefinition reportDefinition,Map<String, Object> parameters){
    	//报表没有设置SQL
        if(Collections3.isEmpty(reportDefinition.getDatasources()) ||
                Collections3.isEmpty(reportDefinition.getDatasources().get(0).getDatasets())){
            return;
        }
        ...
        for(DatasetDefinition dsDef : definitions){
            ...
            if(dsDef instanceof  SqlDatasetDefinition){
                ...
                //保存原始SQL
                if(StrUtil.contains(sql,StrUtil.C_COLON+"sqlFilter")){
                ...
                if(sql.startsWith(ExpressionUtils.EXPR_PREFIX) && sql.endsWith(ExpressionUtils.EXPR_SUFFIX)){
                    ...
                    sql = StrUtil.replaceIgnoreCase(sql,StrUtil.COLON+ "sqlFilter",
                            (String)parameters.get("sqlFilter"));
                    ...
                    if(StringUtils.isEmpty((String)parameters.get("sqlFilter"))){
                        sql = StrUtil.replaceIgnoreCase(sql,"and "+StrUtil.COLON+ "sqlFilter",
                                (String)parameters.get("sqlFilter"));
                    }else{
                        sql = StrUtil.replaceIgnoreCase(sql,StrUtil.COLON+ "sqlFilter",
                                (String)parameters.get("sqlFilter"));
